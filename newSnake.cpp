...
void initsnake()
{
    ...
    for(i=1;i<=4;i++)       //设置蛇身，长度为5
    {
        ...
        head->y=5;
        tail=head;          //蛇头变成蛇尾，然后重复循环，此时head和tail都指向蛇头！！！
    }
...
void snakemove()	//蛇前进,上U,下D,左L,右R
{
    ...
    if(status==U)
    {
        ...
        else
        {
            ...
            //因为上面的nexthead将蛇伸长了一个单位，因此要在蛇尾处截断一个单位，此时q指向倒数第二个单位，
            //接下来，要将最后一个单位变为小方块，以保持蛇没吃到食物，长度不变。
